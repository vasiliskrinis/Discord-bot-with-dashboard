...
function overviewPayload(client, db) {
  ...
  const guilds = getGuilds(client).map((guild) => guildSummary(guild, db));
  return {
    ...
    botBans: db.listBotBans(null, 10)
  };
}
...
function guildDetailPayload(client, db, guildId) {
  ...
  return {
    ...
    activeRestrictions: db.listActiveRestrictions(guild.id, 20).map((row) => ({
      userId: row.user_id,
    ...
    recentCases: db.listCases(guild.id, null, 12).map((row) => ({
      caseId: row.case_id,
      type: row.type,
    ...
    ticketPanels: db.listTicketPanels(guild.id).map((row) => ({
    ...
    tickets: db.listTickets(guild.id, null, 20).map((row) => ({
      panelId: row.panel_id,
      userId: row.user_id,
      userLabel: userLabel(client, guild, row.user_id),
    ...
    reminders: db.listReminders(guild.id, 10).map((row) => ({
      userId: row.user_id,
      userLabel: userLabel(client, guild, row.user_id),
      channelLabel: channelLabel(guild, row.channel_id),
      message: row.message,
    ...
    giveaways: db.listGiveaways(guild.id, 10).map((row) => ({
    ...
    aiPrompts: db.listActiveAiPrompts('guild', guild.id, 10),
    state: {
      countingNumber: db.getState(guild.id, 'counting_number', 0),
      countingLastUser: db.getState(guild.id, 'counting_last_user', null),
      robloxSnapshotSaved: Boolean(db.getState(guild.id, 'roblox_versions_snapshot', null)),
      executorSnapshotSaved: Boolean(db.getState(guild.id, 'executor_status_snapshot', null))
    }
  };
}
...
function displayConfigValue(client, guild, key, value) {
  ...
  if (ROLE_LIST_CONFIG_KEYS.has(key)) return value.map((id) => roleLabel(guild, id)).join(', ');
  if (USER_LIST_CONFIG_KEYS.has(key)) return value.map((id) => userLabel(client, guild, id)).join(', ');
  if (key === 'sticky') {
